# -*- coding: utf-8 -*-
"""
Created on Tue Jul 23 12:44:25 2019
...
def Impute(X,k = 5,metric = 'correlation',axis = 0):
    '''
    ...
    # Get NaN map, where is NaN, and which sample needs to be impu    
    _shape = X.shape
    NaN_map = np.isnan(X)
    ...
    for row_id in range(_shape[0]):
        if sample_contains_NaN[row_id]:
            ...
            NaN_col_idx = np.where(np.isnan(X[row_id,:]))[0]# Return a tuple. For only 1 dim, so write a [0]
